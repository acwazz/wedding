...
- **SolidJS SPA** (migrated from TanStack Start/React SSR 2026-09-16): `index.html` static head → `src/entry.tsx` (`render(App, #root)`, client `/invito` fallback) → `src/App.tsx` = ALL page sections (Header, Hero, Program, InfoUtili, Rsvp, Footer) + `Counter` stepper + `InfoIcon` badge as local components. No router (single page).
